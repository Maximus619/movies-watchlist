    ...
    if (searchQuery !== "") {
      ...
      arr = selectedMovie.filter((movieObj) => {
        if (movieObj.media_type === 'tv') {
          ...
          title = movieObj.title.toLowerCase();
        }
                ...
                <tbody >
                  {selectedMovie !== null ? filterarr.map((movieObj, index) => (
                    <tr key={index}>
                      ...
                      <td ><img src={movieObj.poster_path ? `https://image.tmdb.org/t/p/original${movieObj.poster_path}` : posterna} style={{ height: '4rem', width: '3rem' }} alt="MovieImage" /></td>
